...
##  for more details.                                                        ##
##                                                                           ##
##  This script particularly evaluates a model with only pleiotropy          ##
###############################################################################
...
h2y =  as.numeric(args[1]) # heritability of y
h2T =  as.numeric(args[2]) # heritability of T
delta =  as.numeric(args[3]) # coefficient for T
rho = as.numeric(args[4]) # correlation in case of pleitropy
...
n_pop = 50
n_markers = 200
h2y = 0.1 # heritability of y
h2T =  0.05 # heritability of T
delta = 0.01
rho = 0.03
n_replic = 10; # size of replication sample
...
for(i_rep in 1:rep){
    ...
    zeta = mvrnorm(n_markers, mu=mu, Sigma = sigmab) # Coefficients for the markers, one column of y and one for T
    errors = mvrnorm(n_pop, mu=mu, Sigma = sigmae) # one column of errors for y and one for T
    ...
    m_weight_y = sqrt(h2y)/sqrt(0.5*n_markers)
    e_weight_y = sqrt((h2y+h2T*delta^2)/h2y - h2y - delta^2)
    ...
    save(list = ls(all.names = TRUE),file=paste(output)) # save all variables  including the results
}
