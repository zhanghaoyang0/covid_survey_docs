get_correction <- function(IVs, lambda, lambda_se, h2_LDSC, h2_LDSC_se,
  ...
  get_correctedSE <- function(IVs, lambda, lambda_se, h2_LDSC, h2_LDSC_se, alpha_obs, alpha_obs_se, n_exp, n_out, M, Tr, s=1000, sthreshold=0.05, extracheck=T){
    ...
    if (sum(is.na(res))>=0){have_na = T} # add by zhanghy
    res = res%>%na.omit() # add by zhanghy
    for (i in 1:length(res_subsets)){res_subsets[[i]] = res_subsets[[i]]%>%na.omit()} # add by zhanghy
    ...
    if (have_na == T) {needmore = F} # add by zhanghy, if res have na, then drop while needmore part 
    else {
        needmore = F
    ...
    while(needmore){
      ...
      subsets_var = unlist(lapply(res_subsets, function(x) stats::var(x$corrected)))
      subsets_cov = unlist(lapply(res_subsets,  function(x) stats::cov(x$corrected, x$alpha)))
      subsets_var = subsets_var[!is.na(subsets_var)]; subsets_cov = subsets_cov[!is.na(subsets_cov)] # add by zhanghy
  ...
  test_diff = (alpha_obs - alpha_corrected) /  sqrt(alpha_obs_se^2 + se_cov[1]^2 - 2* se_cov[2])
